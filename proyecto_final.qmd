...
La inspección visual de los predictores permitió revisar la coherencia espacial de las capas antes de entrenar el clasificador. La banda VV del evento muestra la respuesta SAR durante la ventana de inundación, mientras que la diferencia temporal entre evento y preevento permite resaltar cambios de retrodispersión potencialmente asociados a superficies húmedas o inundadas.

![Compuesto Sentinel-1 VV durante la ventana del evento sobre el AOI.](outputs/figures/s1_vv_evento_aoi.png){width=75%}

![Diferencia temporal de retrodispersión VV entre evento y preevento.](outputs/figures/cambio_vv_evemto_prevento.png){width=75%}

La composición SAR RGB combina información de VV, VH y diferencia temporal para facilitar la interpretación conjunta de respuestas radar. Este producto no se usa como imagen óptica ni reemplaza las bandas individuales del modelo, sino que funciona como apoyo visual para revisar patrones espaciales de cambio.

![Composición SAR RGB con VV, VH y diferencia temporal de VV.](outputs/figures/sar_rgb_vv_vh_dvv.png){width=75%}

El componente topográfico se incorporó a partir de NASADEM, dado que la elevación condiciona la acumulación temporal de agua en planicies aluviales. Esta capa fue usada como predictor del Random Forest y como insumo para interpretar la distribución altitudinal de la inundación temporal.

![Modelo digital de elevación NASADEM utilizado como predictor topográfico.](outputs/figures/NASADEM.png){width=75%}
...
OPERA DSWx-S1 se mantuvo como referencia satelital auxiliar para orientar el muestreo, no como verdad de campo. Por ello, el documento interpreta las métricas como una comparación frente a un producto externo de agua superficial y no como validación independiente.
...
![Flujo metodológico del proyecto.](outputs/figures/flujo_metodologico.png){width=100%}
...
La comparación entre Google Earth Engine, Python y Julia se planteó como una verificación de reproducibilidad e interoperabilidad. Cada herramienta cumplió un papel diferente dentro del flujo, por lo que no se interpretan como tres modelos espaciales independientes, sino como componentes complementarios de una misma cadena de análisis.

| Herramienta | Rol en el proyecto | Producto principal | Alcance de la comparación |
|---|---|---|---|
| Google Earth Engine | Procesamiento ráster completo y clasificación espacial | Máscara de inundación, severidad, áreas y métricas RF | Resultado principal del análisis |
| Python | Reentrenamiento local con muestras exportadas y postproceso | Métricas RF, importancia de variables, figuras y tablas limpias | Verifica comportamiento del modelo sobre las muestras |
| Julia | Lectura y resumen de salidas tabulares | Resumen de métricas, áreas y porcentajes topográficos | Comprueba interoperabilidad y síntesis tabular |
...
El resumen en Julia recuperó los principales indicadores tabulares derivados del flujo. Esta salida no recalcula la clasificación, sino que verifica que los productos exportados puedan ser leídos y sintetizados en un tercer lenguaje del curso.

| Indicador | GEE | Python | Julia |
|---|---:|---:|---:|
| Exactitud global | 0.859 | 0.859 | 0.859 |
| Kappa | 0.719 | 0.719 | 0.719 |
| Muestras totales | 1800 | 1800 | 1800 |
| Muestras de entrenamiento | 1281 | 1281 | 1281 |
| Muestras de validación | 519 | 519 | 519 |
| Área inundada final (ha) | 31103.13 | 31103.13 | 31103.13 |
| Área 0-5 m de elevación (%) | 80.69 | 80.69 | 80.69 |
| Área 0-2 grados de pendiente (%) | 57.80 | 57.80 | 57.80 |
